...
public partial class CircleFs : Control, ISubf
{
    ...
    public void PrintParticles(ref Node3D obj)
    {
        if (edit == false)
        {
            ...
            float dt = angle * (float)Math.PI / 180;
    ...
    public List<string> ReturnScript(bool childe = false)
    {
        ...
        script.Add("        CircleFS({_l}, loop-value, {_parameters::%{_n}%::*}, " + $"{ReturnLine()[0]}, {ReturnLine()[1]}, {ReturnLine()[2]}, vector(0,0,0), {GetNode<LineEdit>("HBoxContainer2/Delay2").Text})");
        return script;
    }
}
